...
const logs = ref([])
const meta = reactive({ users: [], operation_types: [] })
const filters = reactive({ user: '', operation_type: '' })
...
const relativeFetched = computed(() => relative(lastFetched.value, 'fetched'))

async function loadMeta() {
  try {
    const res = await api.get('/api/core/logs/meta/')
    meta.users = res.data.users || []
...
async function reload() {
  loading.value = true
  try {
    const res = await api.get('/api/core/logs/', {
      params: {
        page_size: 100,
...
async function poll() {
  // Incremental fetch: ask only for entries newer than the latest we have
