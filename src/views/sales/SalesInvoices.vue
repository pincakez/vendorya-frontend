              ...
              <tr v-for="inv in invoices" :key="inv.id" class="dt-row">
                <td v-for="col in displayColumns" :key="col.key" :class="[col.cls, col.align === 'right' ? 'ta-right' : '']">
                  ...
                  <template v-else-if="col.key === 'balance'">
                    <span :class="balanceClass(inv)"><Money :value="inv.grand_total - inv.paid_amount" /></span>
                  </template>
                </td>
...
const TABLE_ID  = 'finance_invoices'
const ADHOC_KEY = 'dt_finance_invoices_adhoc'
const LOCKED    = ['invoice_number']
...
const tableMin = computed(() => displayColumns.value.reduce((a, c) => a + colWidths[c.key], 0) + 100)
...
function resetLayout() {
  ...
  applyLayout(baseConfig.value); fetchInvoices(1)
}
...
function toggleHidden(key) {
  ...
  const i = working.hidden.indexOf(key); if (i >= 0) working.hidden.splice(i, 1); else working.hidden.push(key)
}
...
function goPage(p) { if (p < 1 || p > totalPages.value) return; fetchInvoices(p) }

let searchTimer = null
function debouncedFetch() { clearTimeout(searchTimer); searchTimer = setTimeout(() => fetchInvoices(1), 300) }
function clearSearch() { search.value = ''; fetchInvoices(1) }

async function fetchCustomers() { try { const res = await api.get('/api/auth/customers/'); customers.value = res.data.results ?? res.data } catch {} }
async function fetchVariants()  { try { const res = await api.get('/api/inventory/variants/', { params: { page_size: 200 } }); variants.value = res.data.results ?? res.data } catch {} }
...
const modal  = reactive({ open: false, customer: '', date: '', discount: 0, status: 'DRAFT', items: [] })
...
function openModal() {
  ...
  Object.assign(modal, { open: true, customer: '', date: now.toISOString().slice(0, 16), discount: 0, status: 'DRAFT', items: [] })
}
...
async function saveInvoice() {
  ...
  try {
    await api.post('/api/finance/invoices/', {
      customer: modal.customer, branch: auth.user?.store?.id, date: modal.date,
      discount: modal.discount || 0, status: modal.status,
