    ...
    <div v-if="openShift" class="shift-banner open">
      <div class="banner-left">
        ...
        <div>
          ...
          <div class="banner-sub">Started {{ fmtDateTime(openShift.start_time) }} · Starting cash: {{ auth.currency }} {{ fmtNum(openShift.starting_cash) }}</div>
        </div>
      </div>
        ...
        <table v-else class="data-table">
          ...
          <tbody>
            ...
            <tr v-for="s in shifts" :key="s.id" class="table-row">
              ...
              <td class="col-amount">{{ auth.currency }} {{ fmtNum(s.starting_cash) }}</td>
              <td class="col-amount">{{ auth.currency }} {{ fmtNum(s.expected_cash) }}</td>
              <td class="col-amount">{{ auth.currency }} {{ fmtNum(s.closing_cash) }}</td>
              <td :class="diffClass(s.difference)">
                {{ Number(s.difference) >= 0 ? '+' : '' }}{{ auth.currency }} {{ fmtNum(s.difference) }}
              </td>
            </tr>
          </tbody>
        </table>
    ...
    <AppModal :open="closeModal" title="Close Shift" @close="closeModal = false">
      <div style="display:flex;flex-direction:column;gap:14px;">
        <div class="shift-summary">
          <div class="summary-row"><span>Starting Cash</span><span>{{ auth.currency }} {{ fmtNum(openShift?.starting_cash) }}</span></div>
          <div class="summary-row"><span>Expected Cash</span><span>{{ auth.currency }} {{ fmtNum(openShift?.expected_cash) }}</span></div>
        </div>
        <div>
        ...
        <div v-if="countedCash !== ''" class="diff-preview" :class="diffClass(countedCash - (openShift?.expected_cash || 0))">
          Difference: {{ Number(countedCash - (openShift?.expected_cash || 0)) >= 0 ? '+' : '' }}{{ auth.currency }} {{ fmtNum(countedCash - (openShift?.expected_cash || 0)) }}
        </div>
      </div>
      <template #footer>
